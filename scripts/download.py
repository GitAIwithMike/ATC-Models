...
def download_file_from_bucket(client, bucket_name, save_dir, audio_path):
    ...
    print(f"Attempting to download: {audio_path}")
    ...
    if not os.path.exists(audio_save_path):
        ...
        try:
            ...
            if err.code == 'NoSuchKey':
                print(f"Object does not exist: {audio_path}")
                return
            else:
        ...
        try:
            print(f"Downloading file to: {download_dest}")
            client.fget_object(bucket_name, download_file, download_dest)
        except S3Error as err:
...
def download_raw_folder(client, bucket_name, save_dir, audio_path):
    ...
    if not os.path.exists(download_file_local):
        ...
        # Download zip file to save directory
        try:
            print(f"Downloading raw folder to: {download_dest}")
            client.fget_object(bucket_name, download_file, download_dest)
        except S3Error as err:
            # Exit if download fails
            print(f'Failed to download {audio_path}')
...
if args.option == 'Date_Range' or args.option == 'All':
    ...
    raw_audio_paths = [
        'Raw/kbtp/2020/Sept-Oct', 'Raw/kbtp/2020/Nov-Dec', 
        'Raw/kbtp/2021/Jan-Feb', 'Raw/kbtp/2021/Mar-Apr', 
        'Raw/kbtp/2021/Nov-Dec', 'Raw/kbtp/2022/Jan-Feb', 
        'Raw/kbtp/2022/Mar-Apr', 'Raw/kbtp/2022/May-Jun', 
        'Raw/kbtp/2022/Jul-Aug', 'Raw/kbtp/2022/Sept-Oct', 
        'Raw/kagc/2021/Oct-Dec', 'Raw/kagc/2022/Jan-Feb', 
        'Raw/kagc/2022/Mar-Apr', 'Raw/kagc/2022/May-Jun', 
        'Raw/kagc/2022/Jul-Aug', 'Raw/kagc/2022/Sept-Oct', 
        'Raw/kagc/2022/Nov-Dec', 'Raw/kagc/2023/Jan-Feb'
    ]
